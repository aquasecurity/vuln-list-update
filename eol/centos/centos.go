...
func (c Config) Update() error {
	...
	for _, cycle := range cycles {
		// CentOS Stream doesn't have security advisories. We shouldn't include it in EOL dates.
		if strings.Contains(strings.ToLower(cycle.Cycle), "stream") {
			continue
		}
		osVer := strings.TrimPrefix(strings.TrimPrefix(cycle.Cycle, "CentOS "), "Linux ")
		if eol, ok := cycle.Eol.(string); ok {
			d, err := time.Parse("2006-01-02", eol)
			if err != nil {
				return xerrors.Errorf("unable to parse %q date: %w", cycle.Eol, err)
			}
			eolDates[osVer] = eolutils.MoveToEndOfDay(d)
		}
	}
